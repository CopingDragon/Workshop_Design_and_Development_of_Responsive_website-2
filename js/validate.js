$(document).ready(function () {
  function validateInput(input) {
    ...
    var rawFieldName = field.attr("name") || "";
    var normalizedFieldName = rawFieldName.replace(/\[\]/g, "");
    var errorTarget =
      field.data("error") || "#" + normalizedFieldName + "_error";
    var errorfield = $(errorTarget);
    var validationType = field.data("validation");
    var minLength = field.data("min") || 0;
    var maxLength = field.data("max") || 9999;
    var fileSizeKb = field.data("filesize") || 0;
    var fileSizeMb = field.data("filesizeMb") || 0;
    ...
    if (validationType) {
      ...
      if (validationType.includes("required")) {
        ...
        } else if (value === "" || value === "-1" || value === null) {
      ...
      if (isFileInput && field[0].files && field[0].files.length > 0) {
        ...
        if (validationType.includes("fileSize")) {
          ...
          if (maxSizeBytes > 0 && oversizeFile) {
            errorMessage = fileSizeMb
              ? `File size must be less than ${fileSizeMb}MB.`
              : `File size must be less than ${fileSizeKb}KB.`;
          }
        }
